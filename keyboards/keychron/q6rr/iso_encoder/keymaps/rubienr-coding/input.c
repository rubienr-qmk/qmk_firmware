   ...
   */
    switch (keycode) {
        ...
        case PR_EMOJI: // ----- print known emoji names
            if (record->event.pressed) {
                const size_t num_emojis = sizeof(ucis_symbol_table) / sizeof(ucis_symbol_t);
                for (size_t idx = 0; idx < num_emojis; idx++) {
                    send_string(ucis_symbol_table[idx].symbol);
                    send_char(' ');
                    wait_ms(20);
                    register_ucis(ucis_symbol_table[idx].code_points);
                    wait_ms(20);
                    send_char('\n');
                }
            }
